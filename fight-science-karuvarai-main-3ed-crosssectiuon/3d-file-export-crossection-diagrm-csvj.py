#pip install trimesh numpy csvkit os-sys
#pip install trimesh numpy pillow csvkit
...
def save_cross_sections(mesh, step, output_dir):
    ...
    for plane in ['xy', 'xz', 'yz']:
        ...
        if plane == 'xy':
            axis_index = 2  # We slice along z (fixed x and y)
            slice_min, slice_max = min_coord[axis_index], max_coord[axis_index]
        elif plane == 'xz':
            axis_index = 1  # We slice along y (fixed x and z)
            slice_min, slice_max = min_coord[axis_index], max_coord[axis_index]
        elif plane == 'yz':
            axis_index = 0  # We slice along x (fixed y and z)
            slice_min, slice_max = min_coord[axis_index], max_coord[axis_index]
